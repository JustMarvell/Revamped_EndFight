...
# Restore exitportal marker if missing (runs every tick but the unless entity check makes it near-free)
...
# EARLY EXIT - skip everything below if no players are in the End
execute in the_end unless entity @a run return 0
...
#music
execute in the_end positioned 0 80 0 as @a[scores={trueEnding_music=0..},distance=..180] at @s run function true_ending:music
